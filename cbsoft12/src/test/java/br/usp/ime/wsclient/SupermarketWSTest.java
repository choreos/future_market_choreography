...
public class SupermarketWSTest {
    ...
    @Test
    public void supermarketShouldReturnAPriceWhenSearchForAProduct() throws Exception {
	WSClient client = new WSClient(SM1_WSDL);
    ...
    @Test
    public void supermarketShouldReturnThePaymentStatusWhenPurchaseAProduct() throws Exception {
	WSClient client = new WSClient(SM1_WSDL);
	Item personalInfo = createPersonalInfo();
	Item invoice = client.request("purchase", personalInfo).getChild("invoice");
	...
	info.addChild("address").setContent("Lost island");
