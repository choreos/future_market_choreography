...
then  
    ...
    if [ -d "${SMSDIR}" ]
	then
	    find $SMSDIR -mindepth 1 -maxdepth 2 -not -name \*.sh -exec cp -r {} $BACKUPDIR/supermarkets/ \;
    fi
    echo "backup shippers"
    if [ -d "${SHIPPERSDIR}" ]
	then
	    find $SHIPPERSDIR -mindepth 1 -maxdepth 2 -not -name \*.sh -exec cp -r {} $BACKUPDIR/shippers/ \;
    fi
...
[1-3])
    ...
    find $SMSDIR -mindepth 1 -maxdepth 2 -not -name \*.sh | xargs rm -rf
    cp -r ${BASEDIR}/$TESTCASE/supermarkets/* ${SMSDIR}

    #copy shippers dir
        find $SHIPPERSDIR -mindepth 1 -maxdepth 2 -not -name \*.sh | xargs rm -rf
